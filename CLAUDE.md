...
- **WIP `feature/v3.0.15-account-mgmt-audit`**(收紧版,只外事号 5 字段 + Sheet 同步,~250 行):用户要求分批发,操作审计/中央台同步等拆到 v3.0.16-20。本版准备 dev VPS 灰度测试 → 测过 OK 再 push main + tag。
- 计划 6 版顺序:v3.0.15 外事号字段+Sheet → v3.0.16 中央台同步 → v3.0.17 账号吊销 @ inspector → v3.0.18 操作审计 → v3.0.19 闲聊词搬 web → v3.0.20 极简 UI
- main:`v3.0.14`(已发布 — 同名外事号自动加 phone 后缀建独立 Sheets 分页)
