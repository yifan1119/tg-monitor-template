...
| v3.3.2 | 对话告一段落跳过 — 扩 SKIP 词单 + 双层匹配 + 新增 CLOSE_PHRASE outbound 检测:客户反馈外事号说「OK 等通知」「再联系」「拜拜」「商务合作请联系」结束语后,对方应答「好的辛苦了」「OK 收到」,30 分钟仍误推未回复预警。两路:① `config.py:is_trivial_no_reply` 双层匹配 — 短词 < `SKIP_CONTAIN_MIN_LEN=5` 字精确匹配(防业务问句误吞如「上架」吃「什么时候上架?」),长结束语 ≥ 5 字子串匹配(吃标点变体「好的辛苦了。」/「!」);扩 `SKIP_NO_REPLY_TEXTS` 默认词单从 30+ 加到 90+(加告一段落整句:好的辛苦了/OK收到/拜拜/再联系/谢谢辛苦了 等);**清理 v3.0.10 残留「业务短词」段**(支付/支付通道/通道/费率/进群了 — 跟 KEYWORDS 默认列表重叠,精确匹配下单字「打款」会跳未回复不合理);抽 `_default_skip_texts()` 函数让 reload + module-level 同来源。② 新增 `config.CLOSE_PHRASE_TEXTS` 默认 40+ 词(等通知/再联系/拜拜/商务合作请联系/晚点回复 等) + `CLOSE_PHRASE_LOOKBACK=3` + `_default_close_phrases()` + `_contains_any` helper;`database.recent_outbound_texts(peer_id, limit=3)` 用 `ORDER BY timestamp DESC, msg_id DESC LIMIT N`(复用 v3.3.1 tiebreaker);`tasks._no_reply_loop` 在 `is_trivial_no_reply` 之后加 `recent_outbound_texts + _contains_any(close_phrase)` 检测,True 跳过 continue 不推 stage1。③ web `setup.html` 04 业务设置段下加「🔚 对话告一段落跳过」字段(textarea + lookback 输入)+ `web.py` `_settings_data` / `save_settings` 读写;`SAFE_KEYS` 加两字段;`config.reload_if_env_changed` 同步 hot reload。21/21 边界 unit test pass(同秒 A+B / 业务问句不误吞 / 标点变体吃 / 客户应答与我方结束语配合)。**0 schema 变 / 0 migration / 0 重登 / 0 配置变 / 默认 ON**。客户可在 web 后台清空 CLOSE_PHRASE_TEXTS 关闭场景 B 检测 | [0060](docs/adr/0060-v3.3.2-conversation-closed-skip.md) |
...
- **WIP v3.3.2** — 对话告一段落跳过(承 v3.3.1)。客户反馈外事号说「OK 等通知」「再联系」「拜拜」「商务合作请联系」结束语后,对方应答「好的辛苦了」「OK 收到」,30 分钟仍误推未回复预警。两路:① `config.is_trivial_no_reply` 双层匹配 — 短词 < 5 字精确(防业务问句误判),长结束语 ≥ 5 字子串(吃标点变体);扩 `SKIP_NO_REPLY_TEXTS` 默认 40+ 整句;清理 v3.0.10 残留「业务短词」(支付/通道/费率/进群了);② 新增 `CLOSE_PHRASE_TEXTS` 默认 40+ 词 + `database.recent_outbound_texts(peer_id, limit=3)` helper,`tasks._no_reply_loop` 检测我方最近 3 条 outbound 含结束语 → 抑制;③ web `setup.html` + `web.py` 加可配字段。Codex round 1 待跑。21/21 unit test pass。**0 schema 变 / 0 migration / 0 重登 / 0 配置变**。ADR-0060。
