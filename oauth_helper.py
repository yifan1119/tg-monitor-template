...
# v3.0.7: OAuth 失效关键词清单 — 单一来源, sheets.py(自愈匹配)和 dashboard_api.py
# (堵塞诊断卡片)都从这里 import, 避免两处人肉同步导致诊断说"OAuth 失效"但
# 自愈识别不出。匹配时调用方应先 .lower() 再做 substring `in` 检查 (helper 已包好)。
# 关键词都是 Google OAuth 错误响应里实测可见的小写串, 跟 google.auth.exceptions
# .RefreshError message 和 OAuth 2.0 RFC 6749 错误码对齐。
OAUTH_FAIL_MARKERS = (
    "invalid_grant",                  # token 已撤销 / 过期 / 帐号改密码
    "token has been expired",         # google-auth RefreshError 常见 message
    "refresh_error",                  # 通用 google-auth refresh 失败标识
    "credentials do not contain",     # 凭证缺字段(损坏的 token 文件)
    "revoked",                        # OAuth Token revocation 通用词
    "unauthorized_client",            # OAuth 2.0 RFC 6749 错误码
    "invalid_client",                 # OAuth 2.0 RFC 6749 错误码
)


def is_oauth_failure(error_text: str) -> bool:
    """统一 OAuth 失败识别 — 匹配 OAUTH_FAIL_MARKERS 之一即返 True。
    内部统一 .lower(), 调用方传原始异常 message 或日志文本均可。"""
    if not error_text:
        return False
    text = error_text.lower()
    return any(m in text for m in OAUTH_FAIL_MARKERS)
