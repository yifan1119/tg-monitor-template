...
# 用法:
#   curl -fsSL .../install.sh | bash -s -- <COMPANY_NAME> [WEB_PORT] [--https [DOMAIN]]
#   ./install.sh <COMPANY_NAME> [WEB_PORT] [--https [DOMAIN]]
#
# 行为:
#   - 未指定 WEB_PORT 会自动从 5001 起扫描找下一个没被占用的
#   - COMPANY_NAME 已存在会提示是否重装（保留数据）
#   - --https 不带域名 → 自动用 nip.io(IP→域名),装完就是 https://<ip>-nip.io
#   - --https <domain> → 用你自己的域名(需先把 DNS A 记录指到本机)
#   - 不加 --https → 走 http(安装完后也可进设置页再启用)
...
if [ -z "$COMPANY_NAME" ]; then
    echo ""
    echo "用法: $0 <COMPANY_NAME> [WEB_PORT] [--https [DOMAIN]]"
    echo ""
    echo "例如:"
    echo "  $0 yueda                         # HTTP, 自动选端口"
    echo "  $0 dingfeng 5002                 # HTTP, 指定端口"
    echo "  $0 yueda --https                 # HTTPS, 自动 nip.io 域名(装完直接用)"
