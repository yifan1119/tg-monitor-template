...
## 📌 当前版本:v3.1.3.3(2026-05-14)

🔴 **v3.1.3.3:让 v3.1.3.2 的 Caddy 修复真生效 + 中央台升级时同 IP 串行** — Plan agent + Codex 复审抓出 v3.1.3.2 两个 P0:① 改完 Caddyfile 没显式 reload,Caddy 进程内存仍跑老 routes;② 中央台 fanout 并发让同 IP 多部门 VPS 上共用 Caddy 反复 restart 抖动 HTTPS。修法:`update.sh` 5.5b 末尾加 if/else 显式 reload(不吞错不撒谎日志)+ `cp || true` 防 set -e 挂;中央台 v0.22 `fleet.fanout` 加 `serialize_by_ip` 同 IP 部门串行跨 IP 并发,真实域名 dept 退回并发 + log warning。详见 [ADR-0045](docs/adr/0045-v3.1.3.3-caddy-reload-explicit.md)。
