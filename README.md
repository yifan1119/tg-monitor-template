...
> 📌 **最新版**:v3.0.23(2026-05-12) — 🚀 **客户 6 项优化诉求全部完成 + 中央台同步 + UI/文案完善** — 7 个版本累积(v3.0.17 监察员 @ + 异地登录 / v3.0.18 操作审计 / v3.0.19 闲聊词 web / v3.0.20 关动效 / v3.0.21 公司中心拉中央台 / v3.0.22 callback bridge / v3.0.23 UI 完善)。
> - **v3.0.17** 账号失效 @ 监察员 + AuthKeyDuplicated 异地登录单独高优先级文案 + `【外事号离线预警 · 部门 (IP)】` 标题
> - **v3.0.18** 操作审计 audit_logs 表(V8 migration)+ `/audit` 「审批历史」管理员面板(全中文化 + ⚠ 非当事人怠工识别 + 风格统一)
...
> - **v3.0.21** 公司/中心下拉从中央台 `/api/v1/options` 实时拉(60s TTL cache),删 `.env COMPANY_OPTIONS/CENTER_OPTIONS` 单部门字段
> - **v3.0.22** stage2/删除走中央台 callback bridge — `_try_central_route` 加 `callback_meta` 参数 + `_callback_meta_for` 组装 dept_url + metrics_token + buttons;`/api/v1/callback` endpoint 收中央台 POST 处理 4 个 action(violation/cancel/approve/reject)+ audit 写 `expected_actor`(怠工识别)+ Codex P1 fix(本地 fallback 路径补 audit_log)
> - **v3.0.23** UI 文案大改 — navbar 加「📋 审批历史」入口、reLogin 按钮常显 + 自动重启监听释放 session、配置精灵顶部架构提示、添加账号步骤改文案、`_humanize_tg_error` 加 8 类英文 → 中文(database is locked / Cannot send 等)、dashboard 实时告警流 session_revoked → 失效 中文化
> - **跨 repo 配套**:中央台 v0.18(`/api/v1/options` 暴露 + callback_listener daemon + Sheet 同步公式注入修复 + 列宽 autoResize)
> - 部署约束:中央台 `alert_routes.bot_token` ≠ 任何部门 `.env BOT_TOKEN`(避免 polling 撞 409),客户为公司新建独立 bot 才能开启 `CALLBACK_LISTENER_ENABLED=true`(默认 OFF stage2/删除 fallback 本地)
