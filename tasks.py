...
class TaskScheduler:
    ...
    async def _no_reply_stage2_loop(self):
        ...
        行为:
        - 每 60 秒扫一次 get_pending_stage1_alerts(after_minutes=NO_REPLY_STAGE2_AFTER_MIN)
        ...
        - 非工作时段 → 跳过(避免半夜 @ 负责人)
        - 兜底保护:扫到的 stage1 若在 created_at 之后有 outbound,先 mark_stage1_handled_by_reply
          吞掉这条;事件驱动的 listener 钩子若漏了(崩溃 / flush delay),这里兜底
        """
